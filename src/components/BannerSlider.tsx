import React, { useState, useEffect, useRef, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Sparkles, Download, Star, ShieldCheck } from 'lucide-react';
import { AppItem } from '../types';
import { VerifiedBadge } from './AppDetailView';

interface BannerSliderProps {
  apps: AppItem[];
  onSelectApp: (app: AppItem) => void;
  onQuickDownload: (app: AppItem) => void;
  theme: 'dark' | 'light';
}

const AUTO_SLIDE_DURATION = 4500; // 4.5 seconds

export const BannerSlider: React.FC<BannerSliderProps> = ({
  apps = [],
  onSelectApp,
  onQuickDownload
}) => {
  const safeApps = Array.isArray(apps) ? apps : [];
  const featuredApps = safeApps.filter((a) => a && (a.isFeatured || a.isHot)).slice(0, 6);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [direction, setDirection] = useState<number>(1); // 1 = forward, -1 = backward
  const [isPaused, setIsPaused] = useState(false);
  
  const timerRef = useRef<NodeJS.Timeout | null>(null);
  const dragStartX = useRef<number | null>(null);
  const isDragging = useRef<boolean>(false);

  // Next Slide
  const handleNext = useCallback(() => {
    if (featuredApps.length <= 1) return;
    setDirection(1);
    setCurrentIndex((prev) => (prev + 1) % featuredApps.length);
  }, [featuredApps.length]);

  // Prev Slide
  const handlePrev = useCallback(() => {
    if (featuredApps.length <= 1) return;
    setDirection(-1);
    setCurrentIndex((prev) => (prev - 1 + featuredApps.length) % featuredApps.length);
  }, [featuredApps.length]);

  // Auto-slide interval timer (4.5s)
  useEffect(() => {
    if (featuredApps.length <= 1 || isPaused) return;

    timerRef.current = setInterval(() => {
      handleNext();
    }, AUTO_SLIDE_DURATION);

    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
    };
  }, [featuredApps.length, isPaused, handleNext, currentIndex]);

  // Universal Smooth Swipe Handlers (Pointer Events: Works on Touch + Mouse Drag)
  const handlePointerDown = (e: React.PointerEvent) => {
    dragStartX.current = e.clientX;
    isDragging.current = true;
    setIsPaused(true);
  };

  const handlePointerUp = (e: React.PointerEvent) => {
    if (!isDragging.current || dragStartX.current === null) return;
    const diff = dragStartX.current - e.clientX;
    const swipeThreshold = 35; // Responsive smooth threshold

    if (Math.abs(diff) > swipeThreshold) {
      if (diff > 0) {
        // Swiped / Dragged Left -> Next App
        handleNext();
      } else {
        // Swiped / Dragged Right -> Previous App
        handlePrev();
      }
    }

    dragStartX.current = null;
    isDragging.current = false;
    setIsPaused(false);
  };

  const handlePointerCancel = () => {
    dragStartX.current = null;
    isDragging.current = false;
    setIsPaused(false);
  };

  if (featuredApps.length === 0) return null;

  const current = featuredApps[currentIndex] || featuredApps[0];

  return (
    <div
      className="relative w-full rounded-3xl overflow-hidden shadow-2xl border border-purple-500/30 mb-8 select-none touch-pan-y cursor-grab active:cursor-grabbing"
      onPointerDown={handlePointerDown}
      onPointerUp={handlePointerUp}
      onPointerCancel={handlePointerCancel}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      {/* Background with Artwork Banner & Smooth Slide Transitions */}
      <div className="relative h-64 sm:h-72 w-full bg-slate-950 overflow-hidden">
        <AnimatePresence mode="popLayout" custom={direction}>
          <motion.div
            key={current.id || current.name}
            custom={direction}
            initial={{ opacity: 0, x: direction * 50 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: direction * -50 }}
            transition={{ duration: 0.45, ease: [0.25, 1, 0.5, 1] }}
            className="absolute inset-0 w-full h-full"
          >
            <img
              src={current.p1 || current.icon || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=80"}
              alt={current.name}
              className="w-full h-full object-cover filter blur-[2px] opacity-40 scale-105"
            />
          </motion.div>
        </AnimatePresence>

        {/* Cinematic Gradient Overlays */}
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/80 to-transparent pointer-events-none" />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/60 to-transparent pointer-events-none" />

        {/* Foreground Content with Interactive Elements */}
        <div className="absolute inset-0 p-5 sm:p-8 flex flex-col justify-end pointer-events-auto">
          <AnimatePresence mode="wait">
            <motion.div
              key={current.id || current.name}
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              transition={{ duration: 0.3 }}
              className="flex items-start gap-3.5 sm:gap-4"
            >
              <img
                src={current.icon || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=200&auto=format&fit=crop&q=80"}
                alt={current.name}
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl sm:rounded-3xl object-cover border-2 border-purple-500/50 shadow-xl flex-shrink-0 cursor-pointer hover:scale-105 transition-transform"
                onClick={(e) => {
                  e.stopPropagation();
                  onSelectApp(current);
                }}
              />

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="bg-gradient-to-r from-purple-600 to-pink-600 text-white text-[9px] font-black uppercase px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1">
                    <Sparkles className="w-2.5 h-2.5" /> FEATURED VIP
                  </span>
                  <span className="text-[10px] text-purple-400 font-bold uppercase tracking-wider">
                    {current.cat}
                  </span>
                </div>

                <h2
                  onClick={(e) => {
                    e.stopPropagation();
                    onSelectApp(current);
                  }}
                  className="text-lg sm:text-2xl font-black text-white truncate cursor-pointer hover:text-purple-300 transition flex items-center gap-1.5"
                >
                  <span>{current.name}</span>
                  <VerifiedBadge size={18} />
                </h2>

                <p className="text-xs text-slate-300 line-clamp-2 max-w-lg mt-1 font-normal">
                  {current.desc}
                </p>

                {/* Stats & Actions */}
                <div className="flex items-center gap-2.5 sm:gap-4 mt-2.5 sm:mt-3 flex-wrap">
                  <div className="flex items-center gap-1 text-xs text-yellow-400 font-bold">
                    <Star className="w-3.5 h-3.5 fill-yellow-400" />
                    <span>{current.rating || '4.9'}</span>
                  </div>
                  <span className="text-slate-500 text-xs">•</span>
                  <span className="text-xs text-slate-300 font-medium">
                    {current.mb} MB
                  </span>
                  <span className="text-slate-500 text-xs">•</span>
                  <span className="text-xs text-emerald-400 font-medium flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5" /> Verified Safe
                  </span>

                  <div className="flex items-center gap-2 ml-auto">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onSelectApp(current);
                      }}
                      className="py-1.5 px-3 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold backdrop-blur-md transition cursor-pointer"
                    >
                      Details
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onQuickDownload(current);
                      }}
                      className="py-1.5 px-3.5 sm:px-4 rounded-xl bg-gradient-to-r from-amber-500 via-rose-600 to-purple-600 hover:from-amber-400 hover:via-rose-500 hover:to-purple-500 text-white text-xs font-black uppercase tracking-wider shadow-lg shadow-pink-600/30 active:scale-95 transition flex items-center gap-1.5 cursor-pointer"
                    >
                      <Download className="w-3.5 h-3.5" />
                      <span>Install</span>
                    </button>
                  </div>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>

          {/* Minimalist Bottom Indicator Dots */}
          <div className="flex items-center justify-center gap-1.5 mt-3 sm:mt-4">
            {featuredApps.map((_, idx) => (
              <button
                key={idx}
                onClick={(e) => {
                  e.stopPropagation();
                  setDirection(idx > currentIndex ? 1 : -1);
                  setCurrentIndex(idx);
                }}
                className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                  currentIndex === idx
                    ? 'w-7 bg-gradient-to-r from-amber-400 to-purple-500 shadow-md shadow-purple-500/50'
                    : 'w-2 bg-slate-700/80 hover:bg-slate-500'
                }`}
                aria-label={`Slide ${idx + 1}`}
              />
            ))}
          </div>
        </div>
      </div>
    </div>
  );
};
